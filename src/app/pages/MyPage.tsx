...
const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  "진행중": { bg: "bg-[#1F4B43]/10", text: "text-[#1F4B43]" },
...
export function MyPagePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-2xl text-[#1F2623] mb-8" style={{ fontWeight: 700 }}>마이페이지</h1>
        ...
        <div className="w-full lg:w-64 shrink-0">
          <div className="bg-white rounded-2xl p-6 shadow-sm mb-4">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full bg-[#1F4B43]/10 flex items-center justify-center text-lg">
                <span>&#x1F33F;</span>
              </div>
              <div>
                <p className="text-[#1F2623]" style={{ fontWeight: 600 }}>홍길동 님</p>
                <p className="text-xs text-[#1F4B43]" style={{ fontWeight: 600 }}>조합원</p>
              </div>
            </div>
            <div className="text-xs text-[#7A8584] space-y-1">
              <p>가입일: 2024-06-15</p>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-colors cursor-pointer ${
                    tab === t.id ? "bg-[#1F4B43]/10 text-[#1F4B43]" : "text-[#7A8584] hover:bg-[#F7F2E8]"
                  }`}
        ...
        <div className="flex-1">
          {tab === "profile" && (
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm">
              <h2 className="text-lg text-[#1F2623] mb-6" style={{ fontWeight: 700 }}>내 정보</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={f.label}>
                    <p className="text-xs text-[#7A8584] mb-1">{f.label}</p>
                    <p className="text-sm text-[#1F2623]" style={{ fontWeight: 500 }}>{f.value}</p>
                  </div>
                ))}
              </div>
              <button className="mt-8 px-5 py-2.5 rounded-xl border border-[#D6CCBC]/40 text-sm text-[#4A5553] hover:bg-[#F7F2E8] cursor-pointer">
                정보 수정
              </button>
            </div>
          )}
          ...
          {tab === "applications" && !app && (
            <div>
              <h2 className="text-lg text-[#1F2623] mb-6" style={{ fontWeight: 700 }}>내 서비스 신청 내역</h2>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-[#7A8584]">{a.service}</span>
                        </div>
                        <h3 className="text-[#1F2623] group-hover:text-[#1F4B43] transition-colors" style={{ fontWeight: 600 }}>{a.title}</h3>
                        <p className="text-xs text-[#7A8584] mt-1">{a.id} · {a.date}</p>
                      </div>
          ...
          {tab === "applications" && app && (
            <div>
              <button onClick={() => setSelectedApp(null)} className="flex items-center gap-1 text-sm text-[#7A8584] hover:text-[#1F2623] mb-4 cursor-pointer">
                ← 목록으로
              </button>
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs text-[#7A8584]">{app.id}</span>
                </div>
                <h2 className="text-xl text-[#1F2623] mb-2" style={{ fontWeight: 700 }}>{app.title}</h2>
                <p className="text-sm text-[#7A8584] mb-8">서비스: {app.service} · 신청일: {app.date}</p>

                {/* Timeline Stepper */}
                <h3 className="text-sm text-[#1F2623] mb-4" style={{ fontWeight: 700 }}>진행 상태</h3>
                <div className="space-y-4">
                      ...
                      <div key={step} className="flex items-start gap-3">
                        <div className="flex flex-col items-center">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                            done ? "bg-[#1F4B43] text-white" : "bg-gray-100 text-gray-400"
                          }`}>
                            {done ? <CheckCircle2 size={16} /> : <Clock size={16} />}
                          </div>
                          {i < app.steps.length - 1 && (
                            <div className={`w-0.5 h-8 ${done ? "bg-[#1F4B43]" : "bg-gray-200"}`} />
                          )}
                        </div>
                        <div className={`pt-1 ${current ? "text-[#1F4B43]" : done ? "text-[#1F2623]" : "text-[#7A8584]"}`}>
                          <p className="text-sm" style={{ fontWeight: current ? 700 : done ? 500 : 400 }}>
                            {step}
                            {current && <span className="ml-2 text-xs bg-[#1F4B43]/10 px-2 py-0.5 rounded-full">현재</span>}
                          </p>
                        </div>
                      </div>
                    );
          ...
          {tab === "inquiries" && (
            <div>
              <h2 className="text-lg text-[#1F2623] mb-6" style={{ fontWeight: 700 }}>내 문의 내역</h2>
                  ...
                  <div key={inq.id} className="bg-white rounded-xl p-5 shadow-sm">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <h3 className="text-[#1F2623]" style={{ fontWeight: 600 }}>{inq.title}</h3>
                        <p className="text-xs text-[#7A8584] mt-1">{inq.id} · {inq.date}</p>
                      </div>
                      <ChevronRight size={18} className="text-gray-300" />
                    </div>
                  </div>
